...
#### Ej7
##### Identificar finalización de clientes
Para obtener los ganadores primero el servidor debe enterarse que todos los clientes han finalizado de enviar sus bets. Esto ya se hizo en el punto anterior, cada cliente cuando finalice de mandar sus bets enviará un batch vacío para indicar que no tiene más bets a enviar.
##### Winners
Para calcular los ganadores el servidor utiliza la función load bets para ir leyendo del archivo una bet a la vez y fijándose con isWinner si gano. Una vez obtenido los ganadores, envía a cada cliente sus ganadores. Esto lo hace enviando primero la cantidad de ganadores seguido de todos los dni. Los clientes lo reciben y loguean acordemente.  
<img src="./fotos/winners.png" alt="drawing" width="200"/>  
Si bien el servidor solo manda los dni, si el cliente quisiera saber más información sobre el ganador la podría obtener desde su archivo de bets.


#### Ej8
Para hacer que el server trabaje con los clientes en paralelo se necesita trabajar con múltiples hilos de ejecución. Ahora los threads tradicionales, no sirven para paralelismo en python, esto se debe a algo llamado GIL que garantiza que no haya más de un thread ejecutándose en simultáneo. Es por esto que en vez de trabajar con threads, se trabajó con procesos con la asistencia de la biblioteca multiprocessing.  
Una vez se conecta un cliente el proceso principal invoca un nuevo proceso que se encarga de la comunicación con ese cliente. Cada proceso encargado de cada cliente, irá recibiendo los batches de bets como se mostró en los anteriores puntos y escribiendo en memoria las mismas. Ahora debemos sincronizar la escritura. Para esto uso un lock, es decir que cuando alguien quiere escribir una nueva bet primero debe tomar este lock. 
Una vez se conectan todos los clientes, el proceso principal se queda esperando a que los demás procesos le informen que sus respectivos clientes han finalizado. Esto se hace con la ayuda de los objetos WinnerReceiver y WinnerSender que se encargan de encapsular la comunicación entre el proceso principal y los hijos. Cada hijo tiene un pipe bidireccional de comunicación con el padre. El proceso principal tendrá al WinnerSender, y para esperar que se obtengan todas las bets, ejecutará el método wait_for_agencies. Este método lee de cada uno de los pipes de los hijos esperando el número de agencia a la que están atendiendo.Esto será enviado por los procesos hijos únicamente cuando el cliente le pida los ganadores, mandando el número de agencia después del indicador de no más bets a enviar.  
Una vez recibidos todos los números de agencias desde el proceso principal, este calcula los ganadores, y manda a cada proceso hijo los ganadores de sus correspondientes agencias. Los hijos envían los ganadores a los clientes, y terminan.
